...
class FlatListSpider(scrapy.Spider):
    ...
    def parse_detail(self, response):
        ...
        lat = response.css(
            '#adDetailInlineMap::attr(data-lat)').extract_first()
        lon = response.css(
            '#adDetailInlineMap::attr(data-lon)').extract_first()
        price, area = response.css(
            'ul.main-list span strong::text').extract()[:2]
